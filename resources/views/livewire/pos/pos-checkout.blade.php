<div x-data="{ showCart: false }">
    ...
    <div class="flex flex-col lg:flex-row gap-5 h-[calc(100vh-130px)] -mx-2 lg:mx-0">
        ...
        <div class="w-full lg:w-2/3 flex flex-col gap-4 h-full">
            ...
            <div
                class="bg-white p-3 rounded-2xl shadow-sm border border-surface-200 flex flex-col sm:flex-row gap-3 z-10">
                ...
                <div class="relative flex-1 flex gap-2">
                    <div class="relative flex-1">
                        ...
                        <input type="text" wire:model.live.debounce.300ms="search"
                            placeholder="Cari nama produk atau scan barcode/SKU..." id="pos-search-input"
                ...
                <div class="flex gap-2">
                    @if(count($openBills) > 0)
                        <button wire:click="toggleOpenBills"
                            ...
                            Open Bill
                            <span
                                class="absolute -top-1.5 -right-1.5 bg-rose-500 text-white text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center">{{ count($openBills) }}</span>
                        </button>
                    @endif
                </div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto pos-scroll pb-4">
                @if($products->isEmpty())
                    <div class="flex flex-col items-center justify-center h-full text-slate-400 space-y-3">
                        ...
                        @if($search)
                            <button wire:click="$set('search', '')" class="text-xs text-blue-600 hover:underline">Hapus
                                pencarian</button>
                        @endif
                    </div>
                @else
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-3 lg:gap-4 px-1">
                        @foreach($products as $p)
                            ...
                            <div wire:click="{{ $stokKosong ? '' : 'addToCart(' . $p->id . ')' }}" wire:key="prod-{{ $p->id }}-{{ $p->stok_saat_ini }}"
                                ...
                                <div class="aspect-square bg-slate-100 relative overflow-hidden">
                                    @if($p->foto)
                                        <img src="{{ Storage::url($p->foto) }}"
                                            class="w-full h-full object-contain {{ $stokKosong ? 'grayscale' : 'group-hover:scale-105 transition-transform duration-300' }}">
                                    @else
                                    ...
                                    {{-- Badge Stok --}}
                                    <div
                                        class="absolute top-2 right-2 px-2 py-1 rounded-lg text-[10px] font-bold shadow-sm
                                ...
                                <div class="p-3">
                                    <h3 class="text-xs font-semibold text-slate-800 line-clamp-2 leading-tight mb-1"
                                        title="{{ $p->nama_produk }}">
                                        {{ $p->nama_produk }}
                                    </h3>
                                    <p class="text-blue-600 font-bold text-sm truncate"
                                        title="Rp {{ number_format($p->harga_jual, 0, ',', '.') }}">Rp
                                        {{ number_format($p->harga_jual, 0, ',', '.') }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
        ...
        <button @click="showCart = true"
            class="lg:hidden fixed bottom-6 right-4 z-40 bg-blue-600 text-white p-2.5 pr-4 rounded-full shadow-2xl shadow-blue-500/40 flex items-center gap-3 transition-transform animate-fade-in hover:scale-105 active:scale-95 border-2 border-white">
            <div class="relative bg-white/20 p-2 rounded-full w-10 h-10 flex items-center justify-center">
                ...
                <span
                    class="absolute -top-1.5 -right-1.5 bg-rose-500 text-white text-[10px] font-bold w-5 h-5 flex items-center justify-center rounded-full border border-blue-600">{{ $carts->sum('jumlah') }}</span>
            </div>
            <div class="flex flex-col items-start leading-tight min-w-0 max-w-[120px]">
                ...
                <span class="text-sm font-bold truncate w-full">Rp
                    {{ number_format(max(0, $subtotal - $diskonGlobal + $pajakPpn), 0, ',', '.') }}</span>
            </div>
        </button>

        {{-- Mobile Overlay Background --}}
        <div x-cloak x-show="showCart" x-transition.opacity style="display: none;"
            class="fixed inset-0 bg-slate-900/60 z-50 lg:hidden backdrop-blur-sm" @click="showCart = false"></div>
            ...
            <div class="p-4 bg-slate-800 text-white flex justify-between items-center shrink-0">
                <div class="flex items-center gap-3">
                    <button @click="showCart = false"
                        class="lg:hidden w-8 h-8 flex items-center justify-center bg-slate-700 hover:bg-slate-600 rounded-lg text-slate-300 hover:text-white transition-colors">
                ...
                <div class="flex items-center gap-3">
                    <span class="bg-blue-600 text-xs px-2 py-1 rounded-md font-bold">{{ $carts->sum('jumlah') }}
                        item</span>
                    <button wire:click="clearCart" wire:confirm="Kosongkan seluruh keranjang?"
                        class="text-slate-400 hover:text-rose-400 transition-colors" title="Kosongkan keranjang">
                ...
                @else
                    @foreach($carts as $c)
                        <div class="bg-white p-3 rounded-xl border border-slate-100 shadow-sm flex flex-col gap-2 relative group cart-item-enter" wire:key="cart-{{ $c->id }}-{{ $c->jumlah }}"
                            ...
                            {{-- Hapus Item --}}
                            <button wire:click="removeItem({{ $c->id }})"
                                class="absolute top-2 right-2 w-6 h-6 flex items-center justify-center rounded-full bg-rose-50 text-rose-500 hover:bg-rose-500 hover:text-white transition-colors opacity-0 group-hover:opacity-100">
                            ...
                            <div class="pr-6">
                                ...
                                <p class="text-xs text-slate-500 mt-0.5">
                                    ...
                                    @if($c->diskon_item > 0)
                                        <span class="text-rose-500 ml-1">- Rp
                                            {{ number_format($c->diskon_item, 0, ',', '.') }}</span>
                                    @endif
                                </p>
                            </div>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center bg-slate-100 rounded-lg overflow-hidden border border-slate-200">
                                    <button wire:click="updateQuantity({{ $c->id }}, 'decrease')"
                                        class="w-8 h-8 flex items-center justify-center text-slate-600 hover:bg-slate-200 hover:text-rose-600 transition-colors">
                                        <i class="fa-solid fa-minus text-[10px]"></i>
                                    </button>
                                    <span class="w-8 text-center text-xs font-bold text-slate-800">{{ $c->jumlah }}</span>
                                    <button wire:click="updateQuantity({{ $c->id }}, 'increase')"
                                        class="w-8 h-8 flex items-center justify-center text-slate-600 hover:bg-slate-200 hover:text-emerald-600 transition-colors">
                                ...
                                <div class="flex flex-col items-end gap-0.5 min-w-0 max-w-[110px]">
                                    ...
                                    <p class="font-bold text-slate-800 text-sm truncate w-full text-right"
                                        title="Rp {{ number_format($totalItem, 0, ',', '.') }}">Rp
                                        {{ number_format($totalItem, 0, ',', '.') }}</p>
                                    {{-- Tombol edit diskon item (Fase 4.4) --}}
                                    <button @click="editDiskon = !editDiskon"
                                        class="text-[10px] text-blue-500 hover:underline flex items-center gap-0.5 shrink-0">
                            ...
                            <div x-show="editDiskon" x-transition
                                class="flex gap-2 items-center pt-2 border-t border-slate-100">
                                <div class="relative flex-1">
                                    <span class="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400 text-xs">Rp</span>
                                    <input type="number" x-model="diskonInput" min="0" placeholder="Diskon item..."
            ...
            <div class="bg-white border-t border-slate-200 p-4 space-y-3 shrink-0">
                ...
                <div x-data="{ editGlobal: false }">
                    ...
                    <div class="flex justify-between items-center text-sm mt-1">
                        <button @click="editGlobal = !editGlobal"
                            class="text-slate-500 hover:text-blue-600 flex items-center gap-1 transition-colors">
                            <i class="fa-solid fa-tag text-xs"></i>
                            Diskon Global
                        </button>
                        <span class="font-semibold text-rose-500">- Rp
                            {{ number_format($diskonGlobal, 0, ',', '.') }}</span>
                    </div>
                    ...
                    <div x-show="editGlobal" x-transition class="mt-1 flex gap-2">
                        ...
                        <button @click="editGlobal = false"
                            class="text-xs bg-slate-200 text-slate-700 px-2.5 py-1.5 rounded-lg hover:bg-slate-300">OK</button>
                    </div>
                </div>
                ...
                <div class="pt-2 border-t border-slate-100 flex justify-between items-end gap-2">
                    ...
                    <span class="text-xl sm:text-2xl font-bold text-blue-600 truncate text-right"
                        title="Rp {{ number_format(max(0, $totalAkhir), 0, ',', '.') }}">Rp
                        {{ number_format(max(0, $totalAkhir), 0, ',', '.') }}</span>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3 pt-1">
                    <button wire:click="holdBill" wire:loading.attr="disabled"
                        ...
                        <span wire:loading.remove wire:target="holdBill"><i class="fa-solid fa-pause mr-1"></i> Hold
                            Bill</span>
                        <span wire:loading wire:target="holdBill"><i
                    ...
                    <button wire:click="openPaymentModal" wire:loading.attr="disabled"
                        ...
                        <span wire:loading.remove wire:target="openPaymentModal"><i
                                class="fa-solid fa-money-bill-wave mr-1"></i> Bayar</span>
                        <span wire:loading wire:target="openPaymentModal"><i
                                class="fa-solid fa-circle-notch fa-spin"></i></span>
                    </button>
                </div>
            </div>
    ...
    {{-- ===== MODAL PEMBAYARAN ===== --}}
    <div x-data="{ open: @entangle('showPaymentModal') }" x-show="open" style="display: none"
            ...
            <div x-show="open" x-transition:enter="ease-out duration-300"
                ...
                <div class="px-6 py-5 space-y-4">
                    ...
                    <div class="bg-blue-50 border border-blue-100 rounded-xl p-4 text-center px-2">
                        ...
                        @php $totalAkhir = $subtotal - $diskonGlobal + $pajakPpn; @endphp
                        <p class="text-2xl sm:text-3xl font-bold text-blue-800 wrap-break-word">Rp
                            {{ number_format(max(0, $totalAkhir), 0, ',', '.') }}</p>
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-slate-600 mb-1.5">Nama Customer</label>
                            <input type="text" wire:model="namaCustomer"
                ...
                <div class="bg-slate-50 px-6 py-4 flex items-center justify-end gap-3 border-t border-slate-100">
                    ...
                    <button wire:click="processCheckout" wire:loading.attr="disabled" type="button"
                        ...
                        <span wire:loading.remove wire:target="processCheckout"><i class="fa-solid fa-check"></i> Proses
                            Transaksi</span>
                        <span wire:loading wire:target="processCheckout"><i
                                class="fa-solid fa-circle-notch fa-spin"></i> Memproses...</span>
                    </button>
                </div>
            </div>
        </div>
    ...
    <div id="camera-modal" wire:ignore
        class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/60 backdrop-blur-sm px-4">
            ...
            <div class="p-6 relative">
                <div class="relative w-full h-auto min-h-[300px] bg-slate-900 rounded-xl overflow-hidden">
                    ...
                    <div id="scanner-loading"
                        class="absolute inset-0 flex flex-col items-center justify-center text-slate-400 gap-3 bg-slate-900 pointer-events-none">
                ...
                <div id="scan-feedback"
                    class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-emerald-500 text-white px-4 py-2 rounded-lg font-bold shadow-xl hidden flex-col items-center gap-1 z-10">
    ...
    @if($showReceiptModal && $receiptOrderId)
        <div
            class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm px-4 animate-fade-in">
            <div class="bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div class="p-6 bg-slate-100 overflow-y-auto flex-1 flex justify-center">
                    {{-- Area Preview Struk --}}
                    <div class="bg-white shadow-sm border border-slate-200 flex justify-center py-2"
                        style="width: 64mm; min-height: 200px; overflow: hidden;" id="receipt-preview-container">
                        <iframe src="{{ route('kasir.struk', $receiptOrderId) }}" id="receipt-iframe"
                            class="border-none pointer-events-none" style="width: 58mm; min-height: 400px;"></iframe>
                    </div>
                </div>

                <div class="bg-white px-6 py-4 border-t border-slate-100">
                    <div class="grid grid-cols-2 gap-3">
                        <button type="button" onclick="printUSB()"
                            class="flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-semibold transition-colors">
                            <i class="fa-solid fa-print"></i> Cetak USB
                        </button>
                        <button type="button" onclick="printBluetooth('{{ $receiptOrderId }}')"
                            class="flex items-center justify-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-semibold transition-colors">
                            <i class="fa-brands fa-bluetooth-b"></i> Bluetooth
                        </button>
                    </div>
                    <button wire:click="closeReceiptModal"
                        class="mt-3 w-full py-2.5 text-sm font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors">
