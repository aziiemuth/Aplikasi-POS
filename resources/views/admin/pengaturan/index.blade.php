...
<div class="space-y-6 animate-fade-in">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="bg-linear-to-r from-blue-600 to-blue-700 px-6 py-4 flex items-center gap-3">
            <div class="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center">
                <i class="fa-solid fa-store text-white text-lg"></i>
            </div>
            <div>
                <h2 class="text-white font-bold text-lg">Identitas Toko &amp; Kustomisasi Struk</h2>
                <p class="text-blue-100 text-sm">Nama toko, alamat, logo, dan pesan footer struk</p>
            </div>
        </div>

        <form action="{{ route('admin.pengaturan.identitas') }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="nama_toko" id="nama_toko"
                        ...
                        class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-100 outline-none transition-all"
                ...
                <div>
                    ...
                    <input type="number" name="pajak_default" id="pajak_default" min="0" max="100" step="0.1"
                        ...
                        class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-100 outline-none transition-all"
                        placeholder="0">
                    <p class="text-xs text-slate-400 mt-1">Isi 0 jika tidak ada pajak</p>
                </div>
                    ...
                    <div class="flex flex-col sm:flex-row items-start gap-4">
                        ...
                        @if(!empty($settings['logo']))
                            <div class="flex flex-col items-center gap-2 shrink-0">
                                <div class="w-24 h-24 rounded-xl border-2 border-blue-200 bg-slate-50 flex items-center justify-center overflow-hidden">
                        ...
                        @else
                            <div class="w-24 h-24 rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 flex items-center justify-center shrink-0">
                        ...
                        <div class="flex-1">
                            <label for="logo"
                                class="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-slate-300 rounded-xl cursor-pointer bg-slate-50 hover:bg-blue-50 hover:border-blue-300 transition-all group">
                                <i class="fa-solid fa-cloud-arrow-up text-2xl text-slate-400 group-hover:text-blue-500 transition-colors mb-2"></i>
                                <span class="text-sm text-slate-500 group-hover:text-blue-600">Klik untuk upload logo baru</span>
                                <span class="text-xs text-slate-400 mt-1">PNG, JPG, WEBP (maks 2MB)</span>
            ...
            <div class="mt-6 flex justify-end">
                <button type="submit"
                    class="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-xl font-semibold text-sm transition-all shadow hover:-translate-y-0.5">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {{-- BACKUP --}}
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="bg-linear-to-r from-emerald-600 to-emerald-700 px-6 py-4 flex items-center gap-3">
                <div class="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center">
                    <i class="fa-solid fa-database text-white text-lg"></i>
                </div>
                <div>
                    <h2 class="text-white font-bold text-lg">Backup Database</h2>
                    <p class="text-emerald-100 text-sm">Unduh cadangan seluruh data sistem</p>
                </div>
            </div>
            ...
            <div class="p-6">
                ...
                <div class="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-200 mb-4">
                    <div>
                        <p class="text-sm font-semibold text-slate-700">Backup Terakhir</p>
                        <p class="text-xs text-slate-400 mt-0.5">Belum ada info backup tersimpan</p>
                    </div>
                ...
                <a href="{{ route('admin.pengaturan.backup') }}" id="btn-backup"
                    ...
                    class="w-full flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-xl font-semibold text-sm transition-all shadow hover:-translate-y-0.5">
        ...
        {{-- IMPORT EXCEL --}}
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="bg-linear-to-r from-violet-600 to-violet-700 px-6 py-4 flex items-center gap-3">
                <div class="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center">
                    <i class="fa-solid fa-file-excel text-white text-lg"></i>
                </div>
                <div>
                    <h2 class="text-white font-bold text-lg">Import Produk via Excel</h2>
                    <p class="text-violet-100 text-sm">Tambah 1000+ produk sekaligus dari file .xlsx</p>
                </div>
            </div>
            ...
            <div class="p-6">
                ...
                <a href="{{ route('admin.pengaturan.import.template') }}"
                    class="w-full flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2.5 rounded-xl font-semibold text-sm transition-all border border-slate-200 mb-4">
                ...
                <form action="{{ route('admin.pengaturan.import.produk') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <label for="file_excel"
                        class="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-slate-300 rounded-xl cursor-pointer bg-slate-50 hover:bg-violet-50 hover:border-violet-300 transition-all group mb-3">
                        <i class="fa-solid fa-file-excel text-2xl text-slate-400 group-hover:text-violet-500 transition-colors mb-2"></i>
                        <span class="text-sm text-slate-500 group-hover:text-violet-600">Pilih file Excel (.xlsx / .xls)</span>
                        <span id="excel-filename" class="text-xs text-slate-400 mt-1">Belum ada file dipilih</span>
                    ...
                    <button type="submit"
                        class="w-full flex items-center justify-center gap-2 bg-violet-600 hover:bg-violet-700 text-white px-6 py-3 rounded-xl font-semibold text-sm transition-all shadow hover:-translate-y-0.5">
    ...
    ======================================================== --}}
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="bg-linear-to-r from-slate-700 to-slate-800 px-6 py-4 flex items-center gap-3">
            <div class="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center">
                <i class="fa-solid fa-headset text-white text-lg"></i>
            </div>
            <div>
                <h2 class="text-white font-bold text-lg">Bantuan &amp; Request Fitur</h2>
                <p class="text-slate-300 text-sm">Hubungi developer langsung via WhatsApp</p>
            </div>
        </div>
