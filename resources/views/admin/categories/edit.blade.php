@extends('layouts.app')
@section('title', 'Edit Kategori: ' . $category->nama_kategori)
@section('page-title', 'Edit Kategori')
@section('page-subtitle', 'Ubah data kategori: ' . $category->nama_kategori)
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-surface-200 overflow-hidden">
        <div class="px-8 py-5 border-b border-surface-100 flex items-center gap-3">
            <div class="w-10 h-10 bg-blue-100 rounded-xl flex items-center justify-center">
                <i class="{{ $category->icon ?? 'fa-solid fa-tag' }} text-blue-600 text-lg"></i>
            </div>
            <div>
                <h2 class="font-bold text-slate-800">{{ $category->nama_kategori }}</h2>
                <p class="text-xs text-slate-400">{{ $category->products()->count() }} produk dalam kategori ini</p>
            </div>
        </div>

        <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="px-8 py-6 space-y-5">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="deskripsi" value="{{ old('deskripsi', $category->deskripsi) }}"
                    placeholder="Deskripsi singkat" class="form-input">
            </div>

            <div>
                <label class="form-label">Icon FontAwesome</label>
                <div class="flex gap-2">
                    <input type="text" name="icon" id="icon-input" value="{{ old('icon', $category->icon ?? 'fa-solid fa-tag') }}"
                        class="form-input flex-1">
                    <div class="w-11 h-11 border border-slate-200 rounded-xl flex items-center justify-center bg-blue-50 shrink-0">
                        <i id="icon-preview" class="{{ old('icon', $category->icon ?? 'fa-solid fa-tag') }} text-blue-600 text-xl"></i>
                    </div>
                </div>
                <div class="flex flex-wrap gap-2 mt-2">
                    @foreach(['fa-solid fa-wine-bottle','fa-solid fa-mug-hot','fa-solid fa-cookie','fa-solid fa-utensils','fa-solid fa-smoking','fa-solid fa-soap','fa-solid fa-box','fa-solid fa-plug','fa-solid fa-tag','fa-solid fa-shirt','fa-solid fa-mobile-screen','fa-solid fa-baby'] as $icon)
            ...
            <div class="flex items-center gap-3">
                ...
                <label for="is_active" class="text-sm text-slate-700 font-medium cursor-pointer">Kategori Aktif</label>
            </div>
...
<script>
function setIcon(icon) { document.getElementById('icon-input').value = icon; document.getElementById('icon-preview').className = icon + ' text-blue-600 text-xl'; }
document.getElementById('icon-input').addEventListener('input', function() { document.getElementById('icon-preview').className = this.value + ' text-blue-600 text-xl'; });
</script>
@endsection
